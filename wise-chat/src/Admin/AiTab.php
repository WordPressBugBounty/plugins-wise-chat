 ...
 */
class AiTab extends AbstractTab {
	...
	private function botAddEditForm($bot = null) {
		...
		$images = $bot ? get_user_meta($bot->ID, 'wc_ai_images', true) : '0';
		?>
                ...
                <table class="wc-bot-form-table wp-list-table widefat">
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <input type="radio" disabled name="wc-bot-type-<?php echo $id; ?>" <?php echo $type === 'assistant' ? 'checked' : ''; ?> value="assistant">
                            <label for="wc-bot-type-assistant-<?php echo $id; ?>">Assistant (Wise Chat Pro with AI only)</label><br />
                            <input type="radio" disabled name="wc-bot-type-<?php echo $id; ?>" <?php echo $type === 'responses' ? 'checked' : ''; ?> value="responses">
                            <label for="wc-bot-type-responses-<?php echo $id; ?>">Responses (Wise Chat Pro with AI only)</label>
                        </td>
                    </tr>
                     <tr>
                    ...
                    <tr>
                        <td class="th-full">Send posted images:</td>
                        <td>
                            <input type="checkbox" disabled id="wc-bot-images-enable-<?php echo $id; ?>" name="wc-bot-images-enable-<?php echo $id; ?>" <?php echo $images === '1' ? 'checked' : ''; ?> value="1">
                            <label for="wc-bot-images-enable-<?php echo $id; ?>">Enable</label><br>
                            <small><strong>Notice 1:</strong> This website must be publicly available because images are sent to AI chatbot as URL. </small><br />
                            <small><strong>Notice 2:</strong> Sending images may be more expensive than sending just texts. Please check the pricing of the model. </small>
                        </td>
                    </tr>
                </table>
                <input type="hidden" name="wc-bot-nonce-<?php echo $id; ?>" value="<?php echo wp_create_nonce($className); ?>" />
                <button type="button" class="button-primary wc-bot-save-button">Save</button>
