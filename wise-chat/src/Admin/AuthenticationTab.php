 ...
 */
class AuthenticationTab extends AbstractTab {
	...
	public function getFields() {
		return array(
			...
			array('_section_ext_go', 'Google Login',
				'In order to setup Google authentication you need to create new project with Google. Then you will be able to get your Client ID and Client Secret. More details <a href="https://kainex.pl/projects/wp-plugins/wise-chat-pro/documentation/external-authentication/google-authentication/" target="_blank">here</a>.<br />
	...
	private static function getAuthModes() {
		return array(
			...
			'external' => array('External', 'Uses external services to authenticate<br/><strong>Notice: </strong> External authentication is available in Wise Chat Pro'),
		);
	}
	...
	public function getProFields() {
		return array(
			...
			'facebook_login_enabled', 'facebook_login_app_id', 'facebook_login_app_secret',
			'twitter_login_enabled', 'twitter_login_api_key', 'twitter_login_api_secret',
			'google_login_enabled', 'google_login_client_id', 'google_login_client_secret',
		);
	}
	...
	public function customFieldsCallback() {
		...
		for ($i = 1; $i <= 7; $i++) {
			...
			foreach ($types as $type => $name) {
				$typesOptions[] = "<option ".(isset($custom[$key]) && $type === $custom[$key]['type'] ? 'selected' : '')." value='{$type}'>{$name}</option>";
			}
			...
			$nameInput = sprintf(
				...
				Options::OPTIONS_NAME, $key, htmlspecialchars(isset($custom[$key]) ? $custom[$key]['name'] : '')
			);
			$typeSelect = sprintf(
				'<select name="%s[auth_username_fields][%d][type]" style="max-width: 100px;">%s</select>',
				Options::OPTIONS_NAME, $key, implode('', $typesOptions)
			);
