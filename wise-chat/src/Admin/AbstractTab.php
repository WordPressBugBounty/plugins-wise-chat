...
use Kainex\WiseChat\DAO\PrivateMessagesRulesDAO;
...
abstract class AbstractTab {
	...
	*/
	 ...
	 * @param PrivateMessagesRulesDAO $privateMessagesRulesDAO
	 ...
	 */
	public function __construct(ChannelsService $channelsService, UserMutesDAO $userMutesDAO, UserService $usersService, MessagesDAO $messagesDAO, ActionsService $actions, FiltersDAO $filtersDAO, NotificationsDAO $notificationsDAO, UserNotificationsDAO $userNotificationsDAO, EmoticonsDAO $emoticonsDAO, PrivateMessagesRulesDAO $privateMessagesRulesDAO, UserMutesService $userMutesService, UserBansService $bansService, MessagesService $messagesService, MessageReactionsService $messageReactionsService, Options $options) {
	...
	*/
	public function radioCallback($args) {
		...
		foreach ($options as $optionValue => $optionDisplay) {
			...
			$disabled = strpos($optionValue, '_DISABLED_') !== false || $isProFeature || $parentId != null && !$this->options->isOptionEnabled($parentId, false);
			if (strpos($optionValue, '_DISABLED_') !== false) {
				$optionLabel .= ' (Wise Chat Pro only)';
			}
	...
	*/
	...
	public function userAddCallback($optionName, $disabled = true) {
		$url = admin_url("options-general.php?page=".Settings::MENU_SLUG."&wc_action=addUserToList&source=".$optionName.'&nonce='.wp_create_nonce('addUserToList'));

		printf(
			'<input type="text" value="" disabled placeholder="User login" id="userSelector%s" %s class="wcUserLoginHint" />'.
