 ...
 */
class FiltersTab extends AbstractTab {
	...
	public function addFilterAction() {
		if (!current_user_can(Settings::CAPABILITY) || !wp_verify_nonce($_GET['nonce'], 'addFilter')) {
			return;
		}
	...
	public function deleteFilterAction() {
		if (!current_user_can(Settings::CAPABILITY) || !wp_verify_nonce($_GET['nonce'], 'deleteFilter')) {
			return;
		}
	...
	public function filterAddCallback() {
		$url = admin_url("options-general.php?page=".Settings::MENU_SLUG."&wc_action=addFilter".'&nonce='.wp_create_nonce('addFilter'));
