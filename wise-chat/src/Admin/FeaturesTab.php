 ...
 */
class FeaturesTab extends AbstractTab {
	...
	public static function getReactionModes() {
		return array(
			...
			'_DISABLED_like_love' => array('Like | Love', 'Display Like and Love buttons'),
			'_DISABLED_like_love_sad' => array('Like | Love | Sad', 'Display Like, Love and Sad buttons'),
			'_DISABLED_custom' => array('Custom', 'Define a custom set of reactions. Please configure up to 7 reactions below.'),
		);
	}
	...
	public function customReactionsCallback() {
		...
		for ($i = 1; $i <= 7; $i++) {
			...
			$idInput = sprintf(
				...
				Options::OPTIONS_NAME, $key, $i
			);
			$actionInput = sprintf(
				'<input type="text" disabled name="%s[reactions_custom][%d][action]" value="%s" maxlength="100" style="max-width: 100px;">%s',
				Options::OPTIONS_NAME, $key, htmlspecialchars($custom[$key]['action'] ?? ''), $i === 1 ? '<p class="description">e.g. Like</p>' : ''
			);
			$activeInput = sprintf(
				'<input type="text" disabled name="%s[reactions_custom][%d][active]" value="%s" maxlength="100" style="max-width: 100px;">%s',
				Options::OPTIONS_NAME, $key, htmlspecialchars($custom[$key]['active'] ?? ''), $i === 1 ? '<p class="description">e.g. I like it</p>' : ''
			);
			...
			$imageInput = sprintf(
				...
				'<button disabled class="wc-image-picker button-secondary" data-target-id="reactions_custom_%d_image" data-image-container-id="reactions_custom_%d_image_container">Browse</button>%s',
				htmlspecialchars($custom[$key]['image']), $i, Options::OPTIONS_NAME, $key, $i, $imageTag, $i, $i, $i === 1 ? '<p class="description">max. 48x48</p>' : ''
			);
			...
			$imageSmInput = sprintf(
				...
				'<button disabled class="wc-image-picker button-secondary" data-target-id="reactions_custom_%d_image_sm" data-image-container-id="reactions_custom_%d_image_sm_container">Browse</button>%s',
				htmlspecialchars($custom[$key]['imageSm']), $i, Options::OPTIONS_NAME, $key, $i, $imageSmTag, $i, $i, $i === 1 ? '<p class="description">max. 48x48</p>' : ''
			);
	...
	public function getProFields() {
		return array('reactions_buttons_group', 'reactions_buttons_mode');
	}

	public function clearAllReactionsAction() {
		$this->messageReactionsService->deleteAll();
		$this->addMessage('All reactions have been deleted');
	}

	public function reactionsActionsCallback() {
		$url = admin_url("options-general.php?page=".Settings::MENU_SLUG."&wc_action=clearAllReactions");

		printf(
			'<a class="button-secondary" href="%s" title="Deletes reactions sent to all messages" onclick="return confirm(\'Are you sure? All reactions will be lost.\')">Clear All Reactions</a>',
			wp_nonce_url($url)
		);
	}
