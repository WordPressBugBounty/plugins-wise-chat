 ...
 */
class EmoticonsTab extends AbstractTab {
	...
	public function getFields() {
		return array(
			...
			array('custom_emoticons_emoticon_width', 'Emoticon Width In Chat', 'selectCallback', 'string', 'Width of a single emoticon in the chat window.', EmoticonsTab::getImageSizes()),
			...
			array('gifs_api_key', 'Tenor GIFs API key', 'stringFieldCallback', 'string', 'Tenor API key <br /><a href="https://kainex.pl/projects/wp-plugins/wise-chat-pro/documentation/features/gifs/" target="_blank">Read more</a>'),
	...
	public function emoticonAddCallback() {
		$url = admin_url("options-general.php?page=".Settings::MENU_SLUG."&wc_action=addEmoticon");
	...
	public function emoticonsCallback() {
		...
		foreach ($emoticons as $key => $emoticon) {
			...
			$actions = array();
			$deleteURL = admin_url("options-general.php?page=".Settings::MENU_SLUG."&wc_action=deleteCustomEmoticon&id=".urlencode($emoticon->getId())).'&wc_tab=emoticons';
			$moveUpURL = admin_url("options-general.php?page=".Settings::MENU_SLUG."&wc_action=moveUpCustomEmoticon&id=".urlencode($emoticon->getId())).'&wc_tab=emoticons';
			$moveDownURL = admin_url("options-general.php?page=".Settings::MENU_SLUG."&wc_action=moveDownCustomEmoticon&id=".urlencode($emoticon->getId())).'&wc_tab=emoticons';
			if ($key > 0) {
				$actions[] = sprintf(
	...
	public function getProFields() {
		return array(
			...
			'custom_emoticons_popup_height', 'custom_emoticons_emoticon_max_width_in_popup', 'custom_emoticons_emoticon_width'
		);
	}
