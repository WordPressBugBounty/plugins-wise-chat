...
class AppearanceTab extends AbstractTab {
	...
	public function getFields() {
		return array(
			...
			array('text_size_chat', 'Font Size', 'selectCallback', 'string', '', AppearanceTab::getFontSizes()),
			...
			array('messages_order', 'Messages Order', 'selectCallback', 'string', 'Sorting order of the messages', AppearanceTab::getSortingOrder()),
			...
			array('text_size', 'Font Size', 'selectCallback', 'string', '', AppearanceTab::getFontSizes()),
			
			array('messages_time_mode', 'Message Time Mode', 'selectCallback', 'string', 'Format of the date and time displayed next to each message', AppearanceTab::getTimeModes()),
			array(
			...
			array('link_wp_user_name', 'Sender Display Mode', 'selectCallback', 'string', '
			    ...
			    ', AppearanceTab::getUserNameLinkModes()),
			...
			array('user_name_prefix', 'User Name Prefix', 'stringFieldCallback', 'string', 'Anonymous user\'s name prefix'),
			...
			array('input_controls_location', 'Input Controls Location', 'selectCallback', 'string', 'Location of the input field, submit button and customizations panel section.', AppearanceTab::getInputFieldLocation()),
			...
			array('sounds_default_muted', 'Sounds Muted by Default', 'booleanFieldCallback', 'boolean', 'Enable if sounds should be muted by default to new visitors.'),
			array(
			...
			array('auto_open_only_existing', 'Auto-open Only Existing Channels', 'booleanFieldCallback', 'boolean', 'By default opens only existing channels on startup'),
			...
			array('text_size_users_list', 'Font Size', 'selectCallback', 'string', 'Font size', AppearanceTab::getFontSizes()),
			...
			array(
				...
				'Dynamic variables: {role}, {id}, {avatar}, {avatar-src}, {name}, {name-linked}, {status}<br />'.
				'<a href="https://kainex.pl/projects/wp-plugins/wise-chat-pro/documentation/templates/" target="_blank">Read about templates</a>'
			),
			array(
				'users_list_info_window_template_name_auth', 'Other users', 'multilineFieldCallback', 'multilinestring',
				'Dynamic variables: {username}, {field1}, {field2}, {field3}, {field4}, {field5}, {field6}, {field7}<br />'.
				'<a href="https://kainex.pl/projects/wp-plugins/wise-chat-pro/documentation/templates/" target="_blank">Read about templates</a>'
			),
			...
			array(
				...
				'Dynamic variables: {role}, {roles}, {id}, {username}, {displayname}, {email}, {firstname}, {lastname}, {nickname}, {description}, {website}, {website-linked}, {avatar}, {avatar-src}, {name}, {name-linked}, {status}, {<i>*user meta field key*</i>}<br />'.
				'<a href="https://kainex.pl/projects/wp-plugins/wise-chat-pro/documentation/templates/" target="_blank">Read about templates</a>'
			),
			array(
				'intro_direct_channel_template', 'User authenticated by name', 'multilineFieldCallback', 'multilinestring',
				'Dynamic variables: {username}, {status}, {field1}, {field2}, {field3}, {field4}, {field5}, {field6}, {field7}<br />'.
				'<a href="https://kainex.pl/projects/wp-plugins/wise-chat-pro/documentation/templates/" target="_blank">Read about templates</a>'
			),
			...
			array(
				...
				"Displays chat button on member profiles. Clicking the button opens a direct chat message window. <br />".
				'<a href="https://kaine.pl/projects/wp-plugins/wise-chat-pro/documentation/features/private-chat-button/" target="_blank">Read more</a>'
			),
	...
	public function getDefaultValues() {
		return array(
			...
			'show_users_list_info_windows' => 0,
			...
			'intro_direct_channel_enabled' => 0,
			'intro_direct_channel_template_wordpress_auth' => "[span className=\"wcName\" content=\"{name}\"] {status}
{role}
{description}",
			'intro_direct_channel_template' => '{username} {status}',
    ...
    public function getParentFields() {
        return array(
	        ...
	        'auto_open_only_existing' => 'auto_open_first_public_channel'
        );
    }
	...
	public static function getAllThemes() {
		return array(
			...
			'colddark' => 'Cold Dark',
			'airflow' => 'Air Flow',
			'_DISABLED_crystal' => 'Crystal',
			'_DISABLED_clear' => 'Clear',
			'_DISABLED_balloon' => 'Balloon'
		);
	}
	...
	public function textColorUserRolesCallback() {
		...
		foreach ($roles as $roleSlug => $roleName) {
			...
			printf(
				'<tr>'.
				'<td>%s:&nbsp;</td><td><input type="text" id="text_color_user_roles_%s" name="'.Options::OPTIONS_NAME.'[text_color_user_roles][%s]" value="%s" class="wc-color-picker" /> </td>'.
				'</tr>',
	...
	public function hiddenUserAddCallback() {
		$this->userAddCallback('users_list_hide_users');
	}

	public function getProFields() {
        return array(
	        'show_users_list_info_windows', 'users_list_info_window_template',
	        'users_list_bp_users_only', 'bp_member_profile_chat_button', 'users_list_info_window_template_ext_auth',
