 ...
 */
class ModerationTab extends AbstractTab {
	...
	public function getFields() {
		return array(
			...
			array(
				'permission_mute_user_role', 'Mute Action', 'checkboxesCallback', 'multivalues',
				'User roles allowed to mute users.<br /> Alternatively, assign "wise_chat_mute_user" capability to any custom role.', self::getRoles()
			),
			array('moderation_mute_user_duration', 'Mute Duration', 'stringFieldCallback', 'integer', 'Empty field means that the user is muted for 1440 minutes (1 day).'),
			array(
				'permission_ban_user_role', 'Ban Action', 'checkboxesCallback', 'multivalues',
	...
	public function deleteModeratorAction() {
		if (!current_user_can(Settings::CAPABILITY) || !wp_verify_nonce($_GET['nonce'], 'deleteModerator')) {
			return;
		}
		if (!isset($_GET['index'])) {
			return;
	...
	public function addModeratorAction() {
		if (!current_user_can(Settings::CAPABILITY) || !wp_verify_nonce($_GET['nonce'], 'addModerator')) {
			return;
		}
		...
		if (!$addModeratorUserLogin) {
			...
			$wpUser = $this->usersService->getWpUserByLogin($addModeratorUserLogin);
			if ($wpUser === null) {
	...
	public function moderatorsCallback() {
		...
		if (count($users) == 0) {
			...
			foreach ($users as $userKey => $userDetails) {
				...
				$deleteURL = $url . '&wc_action=deleteModerator&wc_tab=moderation&index=' . $userKey.'&nonce='.wp_create_nonce('deleteModerator');
				$deleteLink = "<a href='{$deleteURL}' onclick='return confirm(\"Are you sure you want to delete the moderator?\")'>Delete</a><br />";
				$user = $this->usersService->getWpUserByID(intval($userID));
				if ($user !== null) {
	...
	public function moderatorAddCallback() {
		$url = admin_url("options-general.php?page=".Settings::MENU_SLUG."&wc_action=addModerator&nonce=".wp_create_nonce('addModerator'));
		...
		printf(
			...
			'<a class="button-secondary wc-add-moderator-button" href="%s" disabled title="Adds user to the moderators list" style="margin-top: 10px;">Add</a>',
	...
	public function getProFields() {
        return array(
            'enable_edit_own_messages', 'enable_approval_confirmation', 'permission_approve_message_role',
