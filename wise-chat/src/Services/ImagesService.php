...
class ImagesService {
	...
	*/
	 ...
	 */
	public function __construct(Options $options, ImageEditor $imageEditor) {
	...
	*/
	public function getPreparedImage($binaryImageData) {
		...
		if (strlen($binaryImageData) > $this->options->getIntegerOption('images_size_limit', 3145728)) {
			throw new Exception(__('The size of the file exceeds allowed limit.', 'wise-chat'));
		}
		...
		try {
			...
			$this->imageEditor->load($this->tempFileName);
			...
			$this->imageEditor->fixOrientation();
			$this->imageEditor->build();
	...
	private function getTempFileImageInfo($safeResize) {
		...
		if (is_array($fileInfo)) {
			...
			if (!array_key_exists($mimeType, $this->supportedExtensions)) {
				...
				throw new Exception(__('Unsupported type of file', 'wise-chat'));
			}
