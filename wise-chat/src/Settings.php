...
use Kainex\WiseChat\Admin\AbstractTab;
...
class Settings {
	...
	*/
	private $tabs = array(
		...
		'wise-chat-userMutes' => 'Muted Users',
		'wise-chat-bans' => 'Banned Users',
	...
	*/
	...
	public function enqueueScripts() {
		...
		wp_enqueue_style('wise_chat_pro_admin',  plugins_url('../assets/css/wise-chat-pro-admin.css?v='.WISE_CHAT_VERSION, __FILE__));
	}
			...
			<div class="wrap">
				...
				<script type="text/javascript">
					function wise_chat_append_tab(tab) {
						...
						referrer.val(referrer.val() + '#wc_tab=' + tab);
					}
					...
					jQuery(window).load(function() {
						...
						if (location && location.hash && location.hash.length > 0) {
							var matches = location.hash.match(new RegExp('wc_tab=([^&]*)'));
	...
	*/
	public function handleActions() {
		...
		if (isset($_GET['wc_action'])) {
			...
			$redirURL = admin_url("options-general.php?page=".self::MENU_SLUG).(isset($_GET['wc_tab']) ? '#wc_tab='.urlencode($_GET['wc_tab']) : '');
	...
	*/
	private function getTabObject($tabKey) {
		...
		$classPathAndName = "\Kainex\WiseChat\Admin\\".$tabKey."Tab";

		/** @var AbstractTab $tab */
		$tab = Container::getInstance()->get($classPathAndName);
		return $tab;
	}
	...
	public function showDocs() {
		$excludedFields = array(
			...
			'muted_users', 'muted_user_add', 'channels', 'admin_actions', 'filters', 'filter_add',
				...
				if (strpos($id, '_section') === 0) {
					...
					if ($callback == 'booleanFieldCallback') {
						...
						foreach ($values as $key => $value) {
							...
							if ($id == 'permission_delete_message_role' || $id == 'permission_mute_user_role') {
								$value = ucfirst($key);
							}
