...
class Options {
	...
	*/
	...
	public function __construct() {
		$this->baseDir = plugin_dir_url(dirname(__FILE__));
		$this->pluginBaseDir = dirname(dirname(__FILE__));
	...
	public static function storeEngineConfig() {
		...
		if (false === get_transient(self::ENGINE_CONFIG_FRESH_FLAG_TRANSIENT_KEY)) {
	...
	*/
	public function isOptionNotEmpty($property) {
		if (is_array($this->options) && array_key_exists($property, $this->options)) {
			if (is_null($this->options[$property])) {
				return false;
			} else if (is_array($this->options[$property])) {
	...
	*/
	...
	public function onOptionsUpdated($oldValue, $value, $option) {
		delete_transient('wise_chat_pro_wp_users_cache');
	}
	...
	*/
	public function dropAllOptions() {
		...
		delete_option(Options::LAST_NAME_ID_OPTION);
	}
