...
namespace Kainex\WiseChat\Rendering\Filters\Pre;
...
class LinksPreFilter {
	...
	*/
	 ...
	 */
	public function __construct(ImagesService $imagesService, ActionsService $actions, AuthenticationService $authentication) {
			...
			foreach ($matches[0] as $detectedURL) {
				...
				if ($detectAndDownloadImages && preg_match(self::URL_IMAGE_REGEXP, $detectedURL)) {
					...
					try {
						...
						$shortCode = ShortcodeConstructor::getImageShortcode($result['id'], $result['image'], $result['image-th'], $detectedURL);
					} catch (Exception $ex) {
						$regularLink = true;
						$this->actions->publishAction(
                            'showErrorMessage',
                            array('message' => $ex->getMessage()),
                            $this->authentication->getUserIdOrNull()
                        );
					}
				} elseif ($detectYouTubeVideos && preg_match(self::URL_YOUTUBE_REGEXP, $detectedURL, $ytMatches)) {
                    $movieId = array_pop($ytMatches);
                    $shortCode = ShortcodeConstructor::getYouTubeShortcode($movieId, $detectedURL);
